...
#    6. Abre una terminal nueva con el behavior_renderer (TTS → robot habla)
#    7. Corre el nodo de speech en la terminal actual
# =============================================================
...
echo -e "\n${YELLOW}[2/7] Compilando workspace (colcon build)...${NC}"
cd "$SCRIPT_DIR"
colcon build --symlink-install
echo -e "${GREEN}✅ Compilación completada${NC}"

# ── 3. Source del workspace ──────────────────────────────────
echo -e "\n${YELLOW}[3/7] Cargando setup.bash...${NC}"
source "$SCRIPT_DIR/install/setup.bash"
echo -e "${GREEN}✅ Workspace cargado${NC}"

# ── 4. Lanzar bringup NAOqi en terminal nueva ────────────────
echo -e "\n${YELLOW}[4/7] Lanzando bringup NAOqi...${NC}"
if [ -d "$NAOQI_WS" ]; then
...
echo -e "\n${YELLOW}[5/7] Lanzando nodo de detección de emociones...${NC}"
...
# ── 6. Lanzar behavior_renderer en terminal nueva ────────────
echo -e "\n${YELLOW}[6/7] Lanzando behavior_renderer_node...${NC}"
gnome-terminal --title="Behavior Renderer" -- bash -c "
    cd '$SCRIPT_DIR'
    source install/setup.bash
    export DOTENV_PATH='$SCRIPT_DIR/.env'
    VENV_SITE='$VENV_SITE_PACKAGES'
    [ -d \"\$VENV_SITE\" ] && export PYTHONPATH=\"\$VENV_SITE:\${PYTHONPATH:-}\"
    echo '🤖 Lanzando behavior_renderer_node...'
    ros2 run behavior_renderer behavior_renderer_node
    echo 'Presiona Enter para cerrar...'
    read
" &
echo -e "${GREEN}✅ Terminal de behavior_renderer abierta${NC}"

# ── 7. Correr nodo de speech en esta terminal ────────────────
echo -e "\n${CYAN}========================================${NC}"
